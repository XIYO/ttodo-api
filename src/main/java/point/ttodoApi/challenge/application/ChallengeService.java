...
public class ChallengeService {
  ...
  public Page<ChallengeListResult> searchChallengesWithFilter(User user, String keyword,
    ...
    if ("popular".equals(sort)) challenges = challengeRepository.findByVisibilityOrderByParticipantCount(
            ChallengeVisibility.PUBLIC, keyword, pageable);
    else challenges = challengeRepository.findByVisibilityAndFilters(
  ...
  public void partialUpdateChallenge(Long challengeId, UpdateChallengeCommand command) {
    ...
    if (challenge.isActive()) if (command.maxParticipants() != null &&
  ...
  public void deleteChallenge(Long challengeId) {
    ...
    // 진행 중인 챌린지는 삭제 불가
    if (challenge.isActive()) throw new BusinessException(ErrorCode.OPERATION_NOT_ALLOWED, "진행 중인 챌린지는 삭제할 수 없습니다");
  ...
  public InviteLinkResponse getInviteLink(Long challengeId, UUID requesterId) {
    ...
    // 생성자만 초대 링크 조회 가능
    if (!challenge.getCreatorId().equals(requesterId)) throw new ForbiddenException("챌린지 생성자만 초대 링크를 생성할 수 있습니다");
  ...
  @Transactional(readOnly = true)
  public static List<ChallengePolicyResponse> getPolicyOptions() {
    return List.of(
  ...
  private double calculateSuccessRate(Challenge challenge) {
    ...
    if (activeParticipants == 0) return 0.0;
