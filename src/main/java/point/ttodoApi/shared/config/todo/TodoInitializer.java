package point.ttodoApi.shared.config.todo;
...
public class TodoInitializer implements ApplicationRunner {
  ...
  private void createDefaultTodosForMember(Member member) {
    ...
    List<TodoTemplate> defaultTodos = new ArrayList<>();

    // 기본 할일 추가
    defaultTodos.add(TodoTemplate.builder()
            .title("첫 번째 할일 완료하기")
    ...
    // === 개인 카테고리 할일 ===
    {
      TodoTemplate t = TodoTemplate.builder()
              .title("아침 명상하기")
      ...
      t.setAnchorDate(rule.getAnchorDate());
      defaultTodos.add(t);
    }
...

    {
      ...
      rule.setInterval(1);
      rule.setByWeekDays(EnumSet.of(WeekDay.MO));
      rule.setAnchorDate(LocalDate.now());
      t.setRecurrenceRule(rule);
      t.setAnchorDate(rule.getAnchorDate());
      defaultTodos.add(t);
    ...
    todoTemplateRepository.saveAll(defaultTodos);
